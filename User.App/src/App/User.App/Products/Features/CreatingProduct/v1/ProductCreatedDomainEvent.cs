...
namespace User.App.Products.Features.CreatingProduct.v1;
...
{
    public static ProductCreatedDomainEvent Of(
        Guid id,
        string? name,
        Guid categoryId,
        decimal price,
    ...
    )
    {
        return new ProductCreatedValidator().HandleValidation(
            new ProductCreatedDomainEvent(id, name!, categoryId, price, description)
        );
...
internal class ProductCreatedValidator : AbstractValidator<ProductCreatedDomainEvent>
{
